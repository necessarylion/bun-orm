...
describe('Having', () => {
  ...
  it('having equal', async () => {
    const users = await db.table('users').groupBy('users.id').having('email', 'bob@example.com').get()
  ...
  it('having greater than', async () => {
    const users = await db.table('users').groupBy('users.id').having('age', '>', 25).get()

    expect(users.length).toBe(3)
  })

  it('where and having greater than', async () => {
    const users = await db.table('users').where('active', true).groupBy('users.id').having('age', '>', 25).get()
