...
describe('SELECT Query Builder', () => {
  ...
  it('should select with column aliases', async () => {
    const users = await db.select({ user_name: 'name', user_email: 'email' } as any).from('users').get();
  ...
  it('should group by column', async () => {
    const userPostCounts = await db.select(['name', 'COUNT(posts.id) as post_count'])
      .from('users')
  ...
  it('should use table aliases', async () => {
    const users = await db.select(['u.name', 'u.email'])
      .from('users', 'u')
      .get();
